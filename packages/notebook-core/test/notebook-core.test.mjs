...
import { isMarimoNotebook, isPythonPath, playgroundUrl, PLAYGROUND_ORIGIN } from "../dist/index.js";
...
test("rejects a triple-quoted string containing an app declaration", () => {
  const source = '"""\napp = marimo.App()\n"""\n';

  assert.equal(isMarimoNotebook(source), false);
});

test("does not let a type annotation span a newline", () => {
  const source = "import marimo\n\napp: object\nother = marimo.App()\n";
...
test("ignores an aliased import after the first 4 KiB", () => {
  const source =
    "import marimo\n\napp = marimo.App()\n" + "#".repeat(4096) + "\nimport marimo as mo\n";
...
test("builds an embedded playground URL without losing the code hash", () => {
  const source = "import marimo\napp = marimo.App()\n";
  const url = new URL(playgroundUrl(source, { ref: "marimo-anywhere:github.com", theme: "dark" }));
...
test("exports the marimo playground origin", () => {
  assert.equal(PLAYGROUND_ORIGIN, "https://marimo.app");
});
