...
test("matches blob .py views by path shape, on any hostname", () => {
  ...
  assert.equal(gitlabHost.matches(new URL("https://github.com/o/r/blob/main/nb.py")), false);
});
...
test("getSource fetches the raw URL derived from the blob path, keeping ref_type", async () => {
  ...
  assert.deepEqual(calls, ["https://gitlab.com/g/sg/r/-/raw/main/dir/nb.py?ref_type=heads"]);
});

test("getSource returns null when the raw fetch is not ok", async () => {
  installDom();
  stubFetch("nope", { ok: false });
  assert.equal(await gitlabHost.getSource(new URL(globalThis.location.href)), null);
});
