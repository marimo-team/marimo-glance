...
test("isBlobUrl accepts a gitlab.com .py blob view", () => {
  assert.equal(isBlobUrl(new URL("https://gitlab.com/o/r/-/blob/main/nb.py")), true);
});

test("isBlobUrl accepts nested group/subgroup paths", () => {
  assert.equal(isBlobUrl(new URL("https://gitlab.com/g/sg/r/-/blob/main/src/nb.py")), true);
});

test("isBlobUrl ignores a ref_type query and keeps matching on the path", () => {
  assert.equal(isBlobUrl(new URL("https://gitlab.com/o/r/-/blob/main/nb.py?ref_type=heads")), true);
});

test("isBlobUrl rejects non-Python blobs", () => {
  assert.equal(isBlobUrl(new URL("https://gitlab.com/o/r/-/blob/main/readme.md")), false);
});

test("isBlobUrl rejects tree and raw paths", () => {
  assert.equal(isBlobUrl(new URL("https://gitlab.com/o/r/-/tree/main/pkg")), false);
...
test("blobRawUrl preserves ref_type so a branch and same-named tag stay distinct", () => {
  assert.equal(
    blobRawUrl(new URL("https://gitlab.com/o/r/-/blob/test/nb.py?ref_type=heads")),
    "https://gitlab.com/o/r/-/raw/test/nb.py?ref_type=heads",
  );
  assert.equal(
    blobRawUrl(new URL("https://gitlab.com/o/r/-/blob/test/nb.py?ref_type=tags")),
    "https://gitlab.com/o/r/-/raw/test/nb.py?ref_type=tags",
  );
});
